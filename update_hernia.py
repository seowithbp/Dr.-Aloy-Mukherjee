...
with open('hernia-surgery.html', 'r', encoding='utf-8') as f:
    html = f.read()

# 1. Meta Tags
html = re.sub(
    r'<title>.*?</title>',
    r'<title>Best Hernia Surgeon in Delhi | Dr. Aloy Mukherjee</title>',
    html, flags=re.DOTALL
)
html = re.sub(
    r'<meta name="description" content=".*?" />',
    r'<meta name="description" content="Seeking the Best Hernia Surgeon in Delhi? Dr. Aloy Mukherjee provides advanced hernia surgery with precision, minimal pain, faster recovery, and trusted care." />\n    <link rel="canonical" href="https://aloymukherjee.com/hernia-surgery.html" />',
    html, flags=re.DOTALL
)
...
html = re.sub(
    r'<h2 style="font-size: 2\.8rem; font-weight: 800; color: #1a1a1a; margin-bottom: 25px;">Understanding Hernias</h2>.*?</div>',
    understanding_html + '\n                  </div>',
    html, flags=re.DOTALL
)
...
html = re.sub(
    r'<div class="causes-grid">.*?</div>\s*</div>\s*</section>',
    causes_html + '\n          </div>\n      </section>',
    html, flags=re.DOTALL
)
...
html = re.sub(
    r'<div class="symptoms-grid">.*?</div>\s*</div>\s*</section>',
    symptoms_html + '\n          </div>\n      </section>',
...
html = re.sub(
    r'<div class="advanced-treatments-grid fade-in-up">.*?</div>\s*</div>\s*</section>',
    treatments_html + '\n          </div>\n      </section>',
    html, count=1, flags=re.DOTALL
)
...
html = re.sub(
    r'<h2 class="section-title">Why Dr. Aloy Mukherjee is the Best <span>Hernia Surgeon</span> in Delhi\?</h2>.*?<a href="about.html" class="btn btn-primary".*?</a>',
    why_dr_aloy_html,
    html, count=1, flags=re.DOTALL
)
...
html = re.sub(
    r'<div class="faq-container fade-in-up">.*?</div>\s*</div>\s*</section>',
    faq_html + '\n          </div>\n      </section>',
    html, count=1, flags=re.DOTALL
)

with open('hernia-surgery.html', 'w', encoding='utf-8') as f:
    f.write(html)
